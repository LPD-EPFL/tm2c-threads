...
unsigned int iRCCE_wait_list_length(iRCCE_WAIT_LIST *list) {
    iRCCE_WAIT_LISTELEM *elem = list->first;
...
void iRCCE_print_sendlist(iRCCE_WAIT_LIST *list, unsigned int sender) {
    ...
    if (elem != NULL) {
        while (elem != NULL) {
            iRCCE_SEND_REQUEST *s = (iRCCE_SEND_REQUEST *) elem->req;
            //PS_COMMAND *cmd = (PS_COMMAND *) s->privbuf;
            printf("(%d) -> ", s->dest);
            elem = elem->next;
        }
    }
...
void iRCCE_print_waitlist(iRCCE_WAIT_LIST *list, unsigned int sender) {
    ...
    if (elem != NULL) {
        while (elem != NULL) {
            if (elem->type == iRCCE_WAIT_LIST_SEND_TYPE) {
                iRCCE_SEND_REQUEST *s = (iRCCE_SEND_REQUEST *) elem->req;
                //PS_COMMAND *cmd = (PS_COMMAND *) s->privbuf;
                printf("(s:%d) -> ", s->dest);
            }
            else {
                iRCCE_RECV_REQUEST *s = (iRCCE_RECV_REQUEST *) elem->req;
                printf("(r:%d) -> ", s->source);
            }
...
static inline void iRCCE_add_wait_list_generic(iRCCE_WAIT_LIST *list, iRCCE_WAIT_LISTELEM * elem)
{
...
void iRCCE_add_send_to_wait_list(iRCCE_WAIT_LIST *list, iRCCE_SEND_REQUEST * req)
{
	...
	elem = (iRCCE_WAIT_LISTELEM*)malloc(sizeof(iRCCE_WAIT_LISTELEM));
        if (elem == NULL) {
            printf("add_send_to_waitlist: malloc failed"); fflush(stdout);
        }
