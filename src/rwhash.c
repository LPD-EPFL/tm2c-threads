...
inline void rw_entry_fetch_readers(rw_entry_t *rwe, unsigned short *readers) {
...
inline void rw_entry_insert_bucket_entry(bucket_entry_t *bucket_entry, int nodeId, RW rw) {
    ...
    /*WRITING/publishing -------------------------------------------------------------------------------------------------*/
    if (rw == WRITE) {
        if (rw_entry_has_writer(rw_entry)) { /*WRITE AFTER WRITE conflict*/
            //TODO: here the logic for WRITE -> WRITE
            ME;
            PRINTF("[X] %d tries to write %d: WRITE lock by %d {", nodeId, bucket_entry->address, rw_entry->shorts[3]);
            PRINTF(" [new-old = %f] } ", tx_started_timestamp[nodeId] - tx_started_timestamp[rw_entry->shorts[3]]);

            if (contention_manager(nodeId, &rw_entry->shorts[3], WRITE_AFTER_WRITE, bucket_entry->address) == NO_CONFLICT) {
            ...
            if (rw_entry_is_unique_reader(rw_entry, nodeId)) {
            ...
            else { /*WRITE AFTER READ conflict*/
                ...
                if (contention_manager(nodeId, readers, WRITE_AFTER_READ, bucket_entry->address) == NO_CONFLICT) {
                    rw_entry_set_writer(rw_entry, nodeId);
                    PRINTF("NEW");
                }
                else {
        ...
        if (rw_entry_has_writer(rw_entry) && !rw_entry_is_writer(rw_entry, nodeId)) { /*READ AFTER WRITE*/
            //TODO: here the logic for READ AFTER WRITE
            ME;
            PRINTF("[X] %d tries to read %d: WRITE lock by %d {", nodeId, bucket_entry->address, rw_entry->shorts[3]);
            PRINTF(" [new-old = %f] } ", tx_started_timestamp[nodeId] - tx_started_timestamp[rw_entry->shorts[3]]);

            if (contention_manager(nodeId, &rw_entry->shorts[3], READ_AFTER_WRITE, bucket_entry->address) == NO_CONFLICT) {
                //attacker won - old aborted
...
inline void ps_hashtable_print(ps_hashtable_t ps_hashtable) {
    ...
    void ps_hashtable_destroy(ps_hashtable_t ps_hashtable) {
        return;
    }
